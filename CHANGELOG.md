...
### 15:35 — 学习卡片收藏、导航登录态、我的语法库Slug优化
- **StudyFlashcard 背面新增收藏按钮**：显示答案后可在卡片背面直接收藏/取消收藏
- **学习页 `/study` 和复习页 `/review`**：同步接入收藏状态，支持实时切换
- **Header 导航显示登录态**：登录后显示用户邮箱 + 「退出」按钮；未登录显示「登录」
- **字典更新**：zh/en `common` 区块新增 `logout` 键
- **我的语法库新增表单**：移除 Slug 输入框，改为提交时根据标题自动生成（`makeSlug`）
