...
export default function AdminEditGrammarPage() {
  ...
  return (
    <div className="max-w-3xl">
      ...
      <h1 className="mb-6 font-serif text-[clamp(24px,3vw,36px)] font-bold tracking-[-0.02em] text-[#242424]">编辑语法 — {grammar.title}</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <Card className="bg-[#fbfaf8] border border-[#ded8d0] rounded-[18px] ring-0 shadow-none">
          <CardContent className="p-5 space-y-3">
            ...
            <Textarea placeholder="详细解释" value={(form.explanation as string) || ""} onChange={(e) => updateField("explanation", e.target.value)} className="rounded-[12px] min-h-[80px]" />
            <Textarea placeholder="使用场景" value={(form.usage_note as string) || ""} onChange={(e) => updateField("usage_note", e.target.value)} className="rounded-[12px] min-h-[60px]" />
          </CardContent>
        </Card>
        ...
        <Card className="bg-[#fbfaf8] border border-[#ded8d0] rounded-[18px] ring-0 shadow-none">
          <CardContent className="p-5 space-y-3">
            ...
            <Textarea placeholder="日语例句" value={(form.example_jp as string) || ""} onChange={(e) => updateField("example_jp", e.target.value)} className="rounded-[12px] min-h-[60px]" />
            <Textarea placeholder="中文翻译" value={(form.example_cn as string) || ""} onChange={(e) => updateField("example_cn", e.target.value)} className="rounded-[12px] min-h-[60px]" />
        ...
        <Card className="bg-[#fbfaf8] border border-[#ded8d0] rounded-[18px] ring-0 shadow-none">
          <CardContent className="p-5 space-y-3">
            ...
            <Textarea placeholder="常见错误" value={(form.common_mistake as string) || ""} onChange={(e) => updateField("common_mistake", e.target.value)} className="rounded-[12px] min-h-[60px]" />
            <Textarea placeholder="记忆提示" value={(form.memory_tip as string) || ""} onChange={(e) => updateField("memory_tip", e.target.value)} className="rounded-[12px] min-h-[60px]" />
          </CardContent>
        </Card>
